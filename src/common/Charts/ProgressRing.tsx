...
export default function ProgressRing({
  ...
  const handleCopy = () => {
    const text = data.map((item) => `${item.name}: ${item.value}% (${item.count} tasks)`).join("\n")
  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-gray-200 w-[75%] h-full">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-800">{title}</h2>
        <div className="flex gap-2">
          <button onClick={handleCopy} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Copy data">
      ...
      {hoveredIndex !== null && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <div className="flex items-center gap-3">
            <div className="w-4 h-4 rounded-full" style={{ backgroundColor: data[hoveredIndex].color }}></div>
            <span className="text-sm font-medium text-gray-700">{data[hoveredIndex].name}</span>
