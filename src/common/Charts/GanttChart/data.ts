...
import type { ITask, ILink, IScaleConfig, IZoomConfig } from '@svar-ui/gantt-store';

export const weekScaleTemplate = (a: Date, b?: Date) => {
  return `${format(a, 'MMM d')}${b ? ` - ${format(b, 'MMM d')}` : ''}`;
};
...
export const complexScales: IScaleConfig[] = [
...
export const bigScales: IScaleConfig[] = [
  { unit: 'year', step: 1, format: '%Y' },
...
interface IGeneratedTask extends ITask {
  id: number;
}

function getGeneratedTasks(prefix?: string, maxSize?: number, maxYears?: number): ITask[] {
  ...
  for (let i = 1; i <= maxSize; i++) {
    ...
    const start = 1 + ii;
    const end = start + 1 + Math.round(Math.random() * 2);
    tasks.push({
      id: i,
...
const scalesHour: IScaleConfig[] = [
  { unit: 'day', step: 1, format: '%M %j' },
  { unit: 'hour', step: 1, format: '%H:%i' },
];

const critTasks: ITask[] = [
  {
    id: 10,
    start: new Date(2026, 3, 2),
...
const datasets: Record<string, IDataPayload> = {
  day: { tasks: initialTasks, links: initialLinks, scales },
  hour: { tasks: tasksHour, links: linksHour, scales: scalesHour },
  critical: { tasks: critTasks, links: critLinks, scales },
  calendar: { tasks: calendarTasks, links: initialLinks, scales },
};

export function getData(name: string, config?: { baselines?: boolean; splitTasks?: boolean; unscheduledTasks?: boolean }): IDataPayload {
  const source = datasets[name as keyof typeof datasets] || datasets.day;
  // Clone to avoid mutating original datasets
  ...
  if (config?.baselines) {
    data.tasks = data.tasks.map((t: ITask) => {
      if (!t.base_start) {
  ...
  if (config?.splitTasks) {
    const t = data.tasks.find((task) => task.id === 20);
    if (t) {
      const start = t.start || new Date();
  ...
  if (config?.unscheduledTasks) {
    const t = data.tasks.find((task) => task.id === 22);
    if (t) {
      t.unscheduled = true;
...
export function getTypedData(): IDataPayload {
  const t = initialTasks.map((task, i) => {
    const res = { ...task };
    if (res.type == 'task' && i % 3) {
      res.type = taskTypes[(i % 5) + 2].id as string;
    }
    return res;
  });

  return { tasks: t, links: initialLinks, scales };
}

function hoursTemplate(a: Date, b?: Date): string {
  return `${format(a, 'HH:mm')}${b ? ` - ${format(b, 'HH:mm')}` : ''}`;
}
export const zoomConfig: IZoomConfig = {
  level: 3,
  levels: [
    {
      minCellWidth: 200,
    ...
    {
      minCellWidth: 100,
      maxCellWidth: 1000, // Added to match IScaleLevel
      scales: [
        { unit: 'month', step: 1, format: '%F %Y' },
...
export function getGeneratedData(prefix?: string, maxSize?: number, maxYears?: number): IDataPayload {
  const tasks = getGeneratedTasks(prefix, maxSize, maxYears);
  const links: ILink[] = [];
  for (let i = 1; i < tasks.length; i++)
    links.push({
      id: i,
      source: (tasks[i - 1].id as number),
      target: (tasks[i].id as number),
      type: 'e2s',
    });
