...
const RenderStaffAvatars: React.FC<RenderStaffAvatarsProps> = ({ staff }) => {
  return (
    <div className="flex items-center -space-x-2 relative">
      ...
      {staff.length > 3 && (
        <div className="w-8 h-8 absolute -right-0.5 z-10 rounded-full bg-gray-100 text-blue-500 text-xs font-medium flex items-center justify-center border-2 border-blue-500">
          +{staff.length - 3}
        </div>
      )}
    </div>
